...
class NaiveModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.set_defaults(batch_size=64, dataset_mode='translucent',
                            ...
                            n_epochs=10, n_epochs_decay=10, display_ncols=3)  # You can rewrite default values for this model..
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['Normal', 'Rough', 'Depth', 'Scatter', 'Coeffs']
        # specify the images you want to save and display. The program will call base_model.get_current_visuals to save and display these images.

        self.visual_names = ['normal_predict', 'depth_predict_vis', 'rough_predict',
                             'normal_image', 'depth_image_vis', 'rough_image',
                             'scene_image']
        # specify the models you want to save/load to the disk.
        if self.isTrain:
            self.model_save_names = ['Predictor']
            self.model_load_names = []
        else:
            self.model_save_names = []
            self.model_load_names = ['Predictor']
        ...
        if self.isTrain:  # only defined during training time
            ...
            self.optimizer = torch.optim.Adam(self.netPredictor.parameters(), lr=opt.lr, betas=(opt.beta1, 0.999))
    ...
    def imageLoss(self, im1, im2):
        ...
        return torch.mean(torch.sum(torch.abs(im1 - im2) * self.binary_mask, axis=[-1, -2, -3]) / (self.num_pixel * num_channel))
