...
class ResnetEncoderRefine(nn.Module):
    ...
    def forward(self, input):
        ...
        return self.resnet_blocks(torch.cat([BRDF_feature,
                                             light_feature], dim=1))
...
class ImageDecoderRefine(nn.Module):

    def __init__(self, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, isDepth=False):

        super(ImageDecoderRefine, self).__init__()
        if type(norm_layer) == functools.partial:
            use_bias = norm_layer.func == nn.InstanceNorm2d
        else:
            use_bias = norm_layer == nn.InstanceNorm2d
        ...
        for i in range(n_downsampling):  # add upsampling layers
            mult = 2 ** (n_downsampling - i)
            model += [nn.ConvTranspose2d(ngf * mult + 32, int(ngf * mult / 2 + 32),
                                         kernel_size=3, stride=2,
                                         padding=1, output_padding=1,
                                         bias=use_bias),
                      norm_layer(int(ngf * mult / 2 + 32)),
                      nn.ReLU(True)]
        model += [nn.ReflectionPad2d(3)]
        model += [nn.Conv2d(ngf + 32, output_nc, kernel_size=7, padding=0)]
        if isDepth:
            model += [nn.ReLU(True)]
        else:
            model += [nn.Tanh()]
...
class FeatureEncoderRefine(nn.Module):
    def __init__(self, output_dim, ngf=64, norm_layer=nn.BatchNorm2d, isActivation=False):
        ...
        model = []
        model += [nn.Conv2d(ngf * 4 + 32, ngf * 2, kernel_size=3, stride=2, padding=1, bias=use_bias),
                  norm_layer(int(ngf * 2)),
                  nn.ReLU(True),
                  nn.Conv2d(ngf * 2, ngf, kernel_size=3, stride=2, padding=1, bias=use_bias),
                  norm_layer(int(ngf)),
